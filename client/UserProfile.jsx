...
const UserProfile = () => {
  ...
  const followUser = () => {
    ...
    axios
      ...
      .then(() => {
        ...
        getUserList(currentUser.user_id);
  ...
  useEffect(() => {
    if (profileUser && currentUser) {
      getProfileList(profileUser.user_id);
    }
    ...
    if (userList) {
      ...
      if (profileUser.name === currentUser.name) {
        ...
        for (let i = 0; i < userList.length; i++) {
          if (userList[i].name === profileUser.name) {
  ...
  useEffect(() => {
    ...
    axios.get(`db/updatedUser/${state.userId}`)
      .then(({ data }) => {
        setSign(data[0].sign), setDOB(data[0].dob);
      });
  }, []);
    ...
    >
      ...
      >
        <ProfileImg
          ...
          style={{ marginLeft: '150px', padding: '10px', backgroundColor: `${color}`}}
        />
