...
import { CompatibilityInput, CompatibilityButton, TarotCard } from './Styled.jsx';
...
const Compatibility = () => {
    ...
    const zodiacSignsDates = ['Aries (March 21 - April 19)', 'Taurus (April 20 - May 20)', 'Gemini (May 21 - June 20)', 'Cancer (June 21 - July 22)', 'Leo (July 23 - August 22)', 'Virgo (August 23 - September 22)', 'Libra (September 23 - October 22)', 'Scorpio (October 23 - November 21)', 'Sagittarius (November 22 - December 21)', 'Capricorn (December 22 - January 19)', 'Aquarius (January 20 - February 18)', 'Pisces (February 19 - March 20'];
...
axios(options)
    .then(({ data }) => console.log('This is the response given from the compat api', data))
    .catch()
    ...
    {console.log('sign1', sign1)}
            ...
            {/* First Zodiac Sign Input */}
                        <label name="sign-1">Enter first zodiac sign:
                        <CompatibilityInput list="signs" name="myBrowser" defaultValue={sign1} onChange={(e) => {setSign1(e.target.value)}} /></label>
                        <datalist id="signs"  bottom="100%">
                            {zodiacSigns.map((sign) => {return <option value={sign}/>})}
                        </datalist>
             {/* Second Zodiac Sign Input */}
                        <label name="sign-2">Enter second zodiac sign:
                        <CompatibilityInput list="signs" name="myBrowser" onChange={(e) => {setSign2(e.target.value)}} /></label>
                        <datalist id="signs">
                            {zodiacSigns.map((sign) => {return <option value={sign}/>})}
                        </datalist>
            {/* Submit Button */}
            <CompatibilityButton onClick={() => {getCompatibility()} }>Submit &hearts; </CompatibilityButton>
           <br></br>
